<table id="tbCalendar">
    ...
    cacacac
    <caption><a title="{$lang['msg']['prev_month']}" href="{$prevMonthUrl}">«</a>&nbsp;&nbsp;&nbsp;<a
            href="{$nowMonthUrl}">
    ...
    <thead>
        <tr>{for $i = 1; $i <= 7; $i++}<th title="{$lang['week'][$i]}" scope="col">
                <small>{$lang['week_abbr'][$i]}</small></th>{/for}</tr>
    </thead>
    <tbody>
        {php}
        ...
        <tr>{foreach $week as $k => $day}<td>{if isset($arraydate[$day])}<a href="{$arraydate[$day]['Url']}"
                    title="{$arraydate[$day]['Date']} ({$arraydate[$day]['Count']})" target="_blank">{$day}</a>{elseif
                $day}{$day}{/if}</td>{/foreach}
            {if $key == max(array_keys($weeks)) && $k < 6}{str_pad('',(7 - count($week)) * 9,"<td>
                </td>")}{/if}</tr>
        {/foreach}
    </tbody>
</table>