...
(function (b) {
    var d = KClass.create({
        init: function (g) {
            ...
            this.cons = h.conCn
                ? b(this.conId).find(h.conCn)
                : b(this.conId).find(h.conTn);
            this.scrcont = b(this.conId).find(this.scrcol);
            this.levels = this.cons.len;
            this.tabs = h.tabCn
                ? b(this.tabId).find(h.tabCn)
                : b(this.tabId).find(h.tabTn);
            this.texts = h.textCn
                ? b(this.textId).find(h.textCn)
                : b(this.textId).find(h.textTn);
            this.tabitemlen = this.tabs.item(0).node.offsetWidth;
            this.effectFn =
                h.conId && h.effect && this.effects[h.effect]
                    ? this.effects[h.effect]
                    : this.effects.def;
            if (i.tabwr) {
            ...
            if (h.effect == "slide" || h.effect == "fxslide") {
                ...
                this.ml =
                    this.cons.item(0).node[
                        h.vertical == "top" ? "offsetHeight" : "offsetWidth"
                    ];
            } else {
                if (h.effect == "scroll") {
                    this.cont.style.position = "absolute";
        ...
        effects: {
            ...
            slide: function (f, i, h) {
                var g = parseInt(
                    f.cont.style[f.Tol] ? f.cont.style[f.Tol] : "0px",
                );
                f.end = g == h ? 1 : 0;
                if (!f.end) {
                    if (g < h) {
        ...
        timeManger1: function (h) {
            if (h.opt.effect == "def") {
                ...
                if (
                    h.opt.effect == "fxslide" ||
                    (h.opt.effect == "slide" && !h.end)
                ) {
                    var j = h.to;
                    var g = h.scrlen;
                    var i = parseInt(
                        h.cont.style[h.Tol] ? h.cont.style[h.Tol] : "0px",
                    );
                    var f = i == j ? 1 : 0;
                    if (!f) {
        ...
        tabMoveFix: function (f, g) {
            if (
                ...
                this.opt.effect == "fxslide" ||
                this.opt.effect == "scroll"
            ) {
                f.to = "-" + g * this.ml;
                this.end = 0;
            } else {
    ...
    var a = KClass.create({
        init: function (f) {
            ...
            if (g.vertical == "left") {
                h.scWr.childWrap(
                    "<div class='cols' style='position:absolute;width:9999px'><div class='count' style='overflow:hidden;float:left'></div></div>",
                );
            } else {
                h.scWr.childWrap(
                    "<div class='cols' style='position:absolute;height:9999px'><div class='count' style='overflow:hidden;float:left'></div></div>",
                );
            }
            h.fill = 0;
            h.Comp = 0;
            ...
            if (b.B.ie) {
                if (h.vertical == "left") {
                    if (
                        parseInt(h.Wrapchild.css("margin-right")) &&
                        !parseInt(h.Wrapchild.css("margin-left"))
                    ) {
                        this.elemLength =
                            h.Wrapchild.node.offsetWidth +
                            parseInt(h.Wrapchild.css("margin-right"));
                    }
                    if (
                        parseInt(h.Wrapchild.css("margin-left")) &&
                    ...
                    if (
                        parseInt(h.Wrapchild.css("margin-bottom")) &&
                        !parseInt(h.Wrapchild.css("margin-top"))
                    ) {
                        this.elemLength =
                            h.Wrapchild.node.offsetHeight +
                            parseInt(h.Wrapchild.css("margin-bottom"));
                    }
                    if (
                        parseInt(h.Wrapchild.css("margin-top")) &&
        ...
        ISL_ScrDown: function (f) {
            if (f.vertical == "left") {
                ...
                if (
                    !f.loop &&
                    -f.scrObj.Left() > f.finalMovePos - f.PageWidth
                ) {
                    f.disbtright(f);
                    f.Stop = 1;
                    f.finish = 1;
        ...
        ISL_StopDown: function (f) {
            ...
            if (f.vertical == "left") {
                ...
                if (
                    !f.loop &&
                    f.Stop &&
                    -f.scrObj.Top() > f.finalMovePos - f.PageWidth
                ) {
                    setTimeout(function () {
                        clearInterval(f.AutoPlayObj);
                    }, f.interval);
    ...
    function e(j) {
        ...
        h.innerHTML = j
            .substring(j.indexOf(">") + 1, j.lastIndexOf("<"))
            .replace(/^\s+|\s+$/g, "");
        return h;
    }
    function c(f, g) {
