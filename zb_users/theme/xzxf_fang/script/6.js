...
if (zones[0]) {
    if (
        zones[0].getAttribute("time") !== null &&
        zones[0].getAttribute("time") != ""
    ) {
    ...
    for (k = 0, len = zones.length, imp = new Array(); k < len; k++) {
        ...
        if (
            zones[k].getAttribute("imp") !== null &&
            zones[k].getAttribute("imp") != ""
        ) {
    ...
    window.onload = function () {
        for (var i = 0, leng = zones.length; i < leng; i++) {
            ...
            if (
                zone_childs[0] !== null &&
                typeof zone_childs[0] !== "undefined"
            ) {
...
function changeAd() {
    for (g = 0, lenc = zones.length; g < lenc; g++) {
        ...
        if (
            zone_childs[advalue] !== null &&
            typeof zone_childs[advalue] !== "undefined"
        ) {
...
function sendReq(zone_childs) {
    ...
    for (u = 0, lenu = zone_childs.length; u < lenu; u++) {
        ...
        if (zone_childs[u].getAttribute("bannerinfo")) {
            ...
            bannerinfoSet +=
                zone_childs[u].childNodes[0].getAttribute("bannerinfo") + ",";
        }
    }
        ...
        for (var i = 0, pl = p.length; i < pl; i++) {
            if (p[i].nodeName === "DIV") {
                ...
                if (childs_img[0].nodeType == 1) {
                    PHPADM_XmlHttpRequest(
                        childs_img[0].getAttribute("src"),
                        "",
                        "GET",
                    );
                }
