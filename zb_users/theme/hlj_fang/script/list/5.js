...
var mbd_config = {
    ...
    column: {
        // 判断是否为栏目页的正则表达式
        iscurrent:
            /^((https|http):\/\/)?(www\.hljnews\.cn){1}\/(([\s\S]+)\/)?((node_\d+|index).html)$/,
    ...
    article: {
        // 判断是否为文章页的正则表达式
        iscurrent:
            /^((https|http):\/\/)?(www\.hljnews\.cn){1}\/([\s\S]+)\/(content_\d+.html)$/,
    ...
    getArticleId: function () {
        ...
        if (mbd_config.article.common) {
            ...
            var articleID = curWeb.substring(
                curWeb.lastIndexOf("/") + 1,
                curWeb.lastIndexOf("."),
            );
    ...
    getColumnName: function () {
        ...
        if (mbd_config.column.common) {
            // 获取当前位置dom是否存在
            var tagElement = mbd_utils.getElesByClassName(
                mbd_config.column.posTag,
                mbd_config.column.posClass,
            );
            if (typeof tagElement != "undefined" && tagElement.length >= 1) {
                ...
                if (typeof aEles != "undefined" && aEles.length >= 1) {
                    for (i = 0; i < aEles.length; i++) {
                        // 导航条中 首页 不作为栏目
                        var curName = mbd_utils.trim(
                            mbd_utils
                                .getInnerText(aEles[i])
                                .replace(/>|\s+/g, ""),
                        );
                        if (
                            i == 0 &&
...
var mbd_init = function () {
    ...
    if (mbd_utils.isBlank(pageInit)) {
        // cookie中的标识为空，则表示是当前会话中第一次加载
        // 标识写入cookie
        ...
        if (!mbd_utils.isBlank(lastTime)) {
            ...
            if (timeArray.length == 2) {
                ...
                if (lastDate == mbd_utils.getNowFormatDate()) {
                    // 同一天的请求才进行处理
    ...
    if (mbd_config.homePage.pageUrl.indexOf(curAddr) != -1) {
    ...
    } else if (new RegExp(mbd_config.article.iscurrent).test(curAddr)) {
        // 稿件详情页
        ...
        // TODO ......
    } else if (new RegExp(mbd_config.column.iscurrent).test(curAddr)) {
        // 栏目页
        var columnName = mbd_config.getColumnName();
        // 栏目页点击事件（或栏目浏览事件）
...
var mbd_utils = {
    ...
    sendRequest: function (url, data) {
        ...
        if (
            "IE" == mbd_utils.browserType() &&
            mbd_utils.IEVersion() <= 9 &&
