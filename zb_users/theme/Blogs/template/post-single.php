<article id="post-{$article.ID}" class="post-{$article.ID} post type-post status-publish format-standard hentry category-{$article.Category.Alias}">
	...
	<header class="entry-header">
		...
		<div class="single_info">
			<i class="fas fa-user"></i>&nbsp;<a href="{php}Blogs_wenzhanglaiyuan($article->ID){/php}" rel="nofollow" target="_blank">{php}Blogs_wenzhangzuozhe($article->ID){/php}</a>&nbsp;
...
<nav class="nav-single-c"> 	
	<nav class="navigation post-navigation" role="navigation">		
		...
		<div class="nav-links">	
		{if $article.Prev}
		<div class="nav-previous"> <a href="{$article.Prev.Url}" rel="prev"><span class="meta-nav-r" aria-hidden="true"><i class="fas fa-angle-left"></i></span></a> </div>
		{/if}
		{if $article.Next}
		<div class="nav-next"> <a href="{$article.Next.Url}" rel="next"><span class="meta-nav-l" aria-hidden="true"><i class="fas fa-angle-right"></i></span> </a> </div>
		{/if}
		</div>	
	</nav>
</nav>
	...
	<div id="layout-tab">
        ...
        <span class="name"><i class="fas fa-bookmark"></i>&nbsp;相关文章</span>
            <span class="plxiaoshi"><span class="keyword">
            	<i class="fas fa-tags"></i>&nbsp;关键词：{if Count($article.Tags)>0}{foreach $article.Tags as $tag}<a href="{$tag.Url}" target="_blank" rel="tag">{$tag.Name}</a>{/foreach}{/if}</span></span>
        </div>
		<ul class="tab-bd">
